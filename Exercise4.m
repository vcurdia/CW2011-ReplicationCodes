...
% become positive. Interest rate is optimal.
%
...
%% Exercise type
ExerciseName = ['Exercise4_dSP_',int2str(dSP),'_Pers_',num2str(PersSP)];
isBind = (BindBp~=0);
if isBind
...
Mat.OptCP.ZLB.NoCP.G0 = G0j;
Mat.OptCP.ZLB.NoCP.G1 = G1j;
Mat.OptCP.ZLB.NoCP.C = Cj;
Mat.OptCP.ZLB.NoCP.G2 = G2j;
Mat.OptCP.ZLB.NoCP.G3 = G3j;

%% OptCP, NoZLB, NoCP
G0j(ny+nG+nF-1,:) = 0;
idxUpsilon = find(double(jacobian(k_t,Upsilon_t)));
G0j(ny+nG+nF-1,idxUpsilon) = 1;
Cj(ny+nG+nF-1) = -Upsilon_ss;
cv = find(all(G0j(1:ny+nG,:)==0,2)==1);
G0j(cv,:) = -G1j(cv,:);
G1j(cv,:) = 0;
G3j(:,cv) = [];
...
Mat.OptCP.NoZLB.NoCP.G0 = G0j;
Mat.OptCP.NoZLB.NoCP.G1 = G1j;
Mat.OptCP.NoZLB.NoCP.C = Cj;
Mat.OptCP.NoZLB.NoCP.G2 = G2j;
Mat.OptCP.NoZLB.NoCP.G3 = G3j;
