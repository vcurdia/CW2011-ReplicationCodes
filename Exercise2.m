function Exercise2(varargin)

% Exercise 2
%
% Compute the equilibrium under no credit policy and optimal interest rate
% policy. The point is to extract the lagrange multiplier for the zero CB
% lending.
%
% Notation:
%   x_t   refers to x{t}
...
%% Exercise type
ExerciseName = ['Exercise2_dSP_',int2str(dSP),'_Pers_',num2str(PersSP)];
isBind = (BindBp~=0);
if isBind
...
if omega_ss>0
    beta = (delta+1+omega_ss*(delta+(1-delta)*pi_b)-...
        (((delta+1)+omega_ss*(delta+(1-delta)*pi_b))^2-4*delta*...
        (1+omega_ss))^(1/2))/2/delta/(1+omega_ss)/Rd_ss;
elseif omega_ss==0
	beta = 1/Rd_ss;
else
...
lambda_s_ss = mu_p*(1+omega_y)*mu_w_ss*Hbar_ss^(-nu)*(Y_ss/Z_ss)^(1+omega_y)/...
    ((1-tau_ss)*Y_ss*(pi_b*Omega_ss^(1/nu)*psi_b^(-1/nu)+...
    (1-pi_b)*psi_s^(-1/nu))^nu);
lambda_b_ss = Omega_ss*lambda_s_ss;
Lambda_ss = pi_b*lambda_b_ss+(1-pi_b)*lambda_s_ss;
lambdatil_ss = psi*(pi_b*(lambda_b_ss/psi_b)^(1/nu)+...
    (1-pi_b)*(lambda_s_ss/psi_s)^(1/nu))^nu;
...
K_ss = Lambda_ss*mu_p*(1+omega_y)*psi*mu_w_ss/lambdatil_ss*Hbar_ss^(-nu)*...
...
% ZLBRule.NoZLB = Upsilon;
% ZLBRule.ZLB = Rd_t-1;
% ZLBList = fieldnames(MPRule);
% nZLB = length(ZLBList);
...
B_t = ctil_b_t-ctil_s_t-((lambda_b_t/psi_b)^(1/nu)-(lambda_s_t/psi_s)^(1/nu))...
    *(lambdatil_t/psi)^(-(1+nu)/nu)*mu_w_ss*Hbar_ss^(-nu)*...
...
G = vpa([...
	...
	Lambda_t/lambdatil_t*mu_p*(1+omega_y)*psi*mu_w_ss*Hbar_ss^(-nu)*...
        (Y_t/Z_ss)^(1+omega_y)+...
...
F = vpa([...
    pi_b*(1-pi_b)*B_t+delta*b_tL*omegatil_tL*Rd_tL/Pi_t-...
        (1+pi_b*(omegatil_t-1))*b_t;
    pi_b*Cbar_b_ss*lambda_b_t^(-sigma_b)+....
        (1-pi_b)*Cbar_s_ss*lambda_s_t^(-sigma_s)+G_ss+...
...
if rc~=0
    error(['Solution of steady state system is not normal, rc = ',...
        int2str(rc)])
end
...
RatioList = {...
    ...
    'eta_cb','Xitil_cb_ss',...
    'omega_chi','omega_Xi','omega_chiadd','omega_Xiadd','omega_b',...
    'elast_omega_b',...
    'zeta_ss','LMNN_ss'...
    };
xdisp(RatioList)
