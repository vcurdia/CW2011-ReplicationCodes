function IRFPlotCompareExercise5(varargin)
...
% Pol2Plot = {'NoCP'};
% Pol2PlotPretty = {'No Credit Policy'};
...
%     var_plot = {'Y','Pi','RdLevel','omegatil','b','gammacb','cs','cb','LongSP'};
%     var_label = {'Y','\pi','i^d (level)','\omega','b','L^{cb}','c^s','c^b','Long Spread'};
%     scale = [1,4,1,4,1,1,1,1,4]; % annualize inflation and interest rates
...
%% load mat file
load(['Output_Exercise5',FileNameSuffix],'zz','nzz','IRF','csi',...
...
%             RefLevel = 100*zeta_ss;
    ...
    if FigPrint
        if all([FigShape{:}]==[4,3])
            FigName = strrep(sprintf('%sIRF_OptCP_Taylor%s_%s_WithMultipliers.eps',...
                FigPrefix,FileNameSuffix,Sj),'_h','_');
        else
            FigName = strrep(sprintf('%sIRF_OptCP_Taylor%s_%s.eps',...
                FigPrefix,FileNameSuffix,Sj),'_h','_');
        end
%         print('-depsc2',[FigName,'.eps'])
    ...
    if SaveFigData
        if all([FigShape{:}]==[4,3])
            FigName = strrep(sprintf('%sFigData_IRF_OptCP_Taylor%s_%s_WithMultipliers',...
                FigPrefix,FileNameSuffix,Sj),'_h','_');
        else
            FigName = strrep(sprintf('%sFigData_IRF_OptCP_Taylor%s_%s',...
                FigPrefix,FileNameSuffix,Sj),'_h','_');
        end
